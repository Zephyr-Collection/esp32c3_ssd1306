...
static void sntp_service_handler(struct net_socket_service_event *pev)
{
	...
	/* Check if there is an error or if the socket is closed */
	if (pev->event.revents & (ZSOCK_POLLERR | ZSOCK_POLLHUP | ZSOCK_POLLNVAL)) {
		return;
	}
	...
	if (rc != 0) {
		/* Avoid logging error if it's already closed */
		if (rc != -EBADF) {
	...
	/* Close the service - this will also close the socket */
...
int do_sntp(int family)
{
	...
	if (rv < 0) {
		...
		/* If timeout, we need to close manually as handler wasn't called/finished */
		sntp_close_async(&service_sntp_async);
		return -1;
	}
	/* No need to call sntp_close(&ctx) here if sntp_close_async was called,
	 * but it's safe to keep as a catch-all if init failed early.
	 */
	struct timespec tp;
	tp.tv_sec = s_time.seconds;
	tp.tv_nsec = 0;
